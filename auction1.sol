...
function totalsupply() public view returns(uint){
    ...
    function register(uint256 Usertype) public payable {
        ...
        require(Usertype == 1 && AuctionCharge ==1000|| Usertype == 2 && AuctionCharge ==500, " sponsor = 1 || bidder  = 2");
    ...
    function Winner(uint256 id , address payable _to) public payable {
       require(Index[msg.sender] == 1, " only sponsor can ");
    ...
    function AuctionCancel(uint id, address to) public {
         _transferID(_owners[id],to, id);
         
      
    }
     // only bid value will transfer not registration fees
function claimBid(uint id ) public payable returns(bool) {
    require(block.timestamp >= endTime[id], "after auction over or cancelled");
